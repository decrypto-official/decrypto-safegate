...
That is not a hypothetical, and 0.7.0 exists because it happened. The reading shipped in 0.6.0 on a 52-token scan and a list of eight spellings. A 400-token sweep immediately afterwards found sixteen tokens carrying a metadata setter, four spellings the list had never heard of, and **two live mainnet tokens publishing `cannot be present` about metadata their own bytecode can rewrite**. A second sweep, run after those four spellings were adopted, found three more spellings and two more such tokens. A third, after the whole watchlist was adopted, found none in a fresh sample of 400. The honest summary is that two consecutive rounds each produced spellings nobody had written down, so the rate at which this list goes stale is not zero and is not known.
