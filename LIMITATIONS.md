...
**One reading does let that table move an axis, and it is the weakest thing in this document.** Since methodology 0.3.0, metadata mutability is recorded `ABSENT` on an Ethereum contract whose bytecode is fixed and which dispatches none of the metadata-mutating selectors in that same table. The dispatch surface is genuinely complete: fixed bytecode carries the selector of every function it has. Our list of what counts as a metadata mutator is not, and cannot be shown to be. **A contract with a name setter under a spelling nobody has catalogued will read as verified clean on that capability rather than as a gap**, and unlike every other use of the table, that silence is now inside a published number instead of beside it. Two things hold it in: the reading is refused on anything upgradeable, and the list is derived from the gap table in code, so widening one widens the other. Neither makes the list complete. Of 52 Ethereum tokens scanned in September 2026, none carried any such setter under any spelling we tried, which is what the reading rests on and is also exactly as much as it proves.
