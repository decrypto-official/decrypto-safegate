...
7. `presenceIndicatedBy` set correctly. `call-success` means the function existing is the finding; `non-empty-value` means the returned value decides. A `uint256` read under `non-empty-value` counts 0 as absent.
